...
friendsRouter.delete('/:id', extractToken, async (req, res) => {
  try {
    ...
    await User.findOneAndUpdate({_id: user._id}, {$pull: {friends: friendId}});
    await User.findOneAndUpdate({_id: friendId}, {$pull: {friends: user._id}});
